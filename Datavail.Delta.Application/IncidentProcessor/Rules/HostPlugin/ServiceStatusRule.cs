...
namespace Datavail.Delta.Application.IncidentProcessor.Rules.HostPlugin
{
    public sealed class ServiceStatus : IncidentProcessorRule
    {
        ...
        {
            var message = string.Format(ServiceDeskMatchMessage, _serviceName, _serviceStatus, MetricInstanceId, metricThreshold.MatchValue, metricThreshold.Id, Hostname, IpAddress);
            return message;
        }

        protected override string FormatMatchCountServiceDeskMessage(int count, MetricThreshold metricThreshold)
        {
            var message = string.Format(ServiceDeskMatchCountMessage, _serviceName, _serviceStatus, MetricInstanceId, count, metricThreshold.TimePeriod, metricThreshold.MatchValue, metricThreshold.Id, Hostname, IpAddress);
            return message;
        }
