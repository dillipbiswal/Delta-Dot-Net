...
namespace Datavail.Delta.Application.IncidentProcessor.Rules.HostPlugin
{
    public sealed class PhysicalMemoryThresholdRule : IncidentProcessorRule
    {
        ...
        private const string ServiceDeskMessage = "The Delta monitoring application has detected a physical memory {0} threshold breach for {1} (metricInstanceId: {2}).\n\nTotal Bytes: {3} ({4:N0})\nAvailable Bytes: {5} ({6:N0})\nPercentage Available: {7:0.00}%\n\nMetric Threshold: {8}\nFloor Value: {9:N2}\nCeiling Value: {10:N2}\nServer: {11} ({12})\nIp Address: {13}\n";
        private const string ServiceDeskMessageCount = "The Delta monitoring application has detected a physical memory {0} threshold breach for {1} (metricInstanceId: {2}). This has occurred {3} times in the last {4} minutes.\n\nTotal Bytes: {5} ({6:N0})\nAvailable Bytes: {7} ({8:N0})\nPercentage Available: {9:0.00}%\n\nMetric Threshold: {10}\nFloor Value: {11:N2}\nCeiling Value: {12:N2}\nServer: {13} ({14})\nIp Address: {15}\n";
        private const string ServiceDeskMessageAverage = "The Delta monitoring application has detected a physical memory {0} threshold breach for {1} (metricInstanceId: {2}). The average has been {3:0.00} over the last {4} samples.\n\nTotal Bytes: {5} ({6:N0})\nAvailable Bytes: {7} ({8:N0})\nPercentage Available: {9:0.00}%\n\nMetric Threshold: {10}\nFloor Value: {11:N2}\nCeiling Value: {12:N2}\nServer: {13} ({14})\nIp Address: {15}\n";
        private const string ServiceDeskSummary = "P{0}/{1}/Physical memory {2} threshold breach";
        ...
        {
            var message = string.Format(ServiceDeskSummary, IncidentPriority, Hostname, metricTypeDescription);
            return message;
        }
