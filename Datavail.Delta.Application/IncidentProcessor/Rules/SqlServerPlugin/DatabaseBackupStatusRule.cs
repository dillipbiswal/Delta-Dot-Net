...
namespace Datavail.Delta.Application.IncidentProcessor.Rules.SqlServerPlugin
{
    public sealed class DatabaseBackupStatusRule : IncidentProcessorRule
    {
        ...
        private const string ServiceDeskMessage = "The Delta monitoring application has detected that database {0} has breached the {1} threshold (metricInstanceId: {2}).\n\nInstance Name: {10}\nMinutes Since Last Backup: {3}\n\nMetric Threshold: {4}\nFloor Value: {5:N2}\nCeiling Value: {6:N2}\nServer: {7} ({8})\nIp Address: {9}\n";
        private const string ServiceDeskMessageCount = "The Delta monitoring application has detected that database {0} has breached the {1} threshold (metricInstanceId: {2}). This has occurred {3} times in the last {4} minutes.\n\nInstance Name: {12}\nMinutes Since Last Backup: {5}\n\nMetric Threshold: {6}\nFloor Value: {7:N2}\nCeiling Value: {8:N2}\nServer: {9} ({10})\nIp Address: {11}\n";
        private const string ServiceDeskMessageAverage = "The Delta monitoring application has detected the database {0} has breached the {1} threshold (metricInstanceId: {2}). The average has been {3:0.00} over the last {4} samples.\n\nTotal Bytes: {5} ({6:N0})\nMinutes Since Last Backup: {7}\nMetric Threshold: {8}\nFloor Value: {9:N2}\nCeiling Value: {10:N2}\nServer: {11} ({12})\nIp Address: {13}\n";
        private const string ServiceDeskSummary = "P{0}/{1}/{2}/Database minutes since last backup threshold breach for DB {3}";
        ...
        {
            var message = string.Format(ServiceDeskSummary, IncidentPriority, Hostname, _instanceName, _databaseName);
            return message;
        }
        ...
        {
            var message = string.Format(ServiceDeskMessage, _databaseName, metricTypeDescription, MetricInstanceId, _minsSinceLast, metricThreshold.Id, metricThreshold.FloorValue, metricThreshold.CeilingValue, Hostname, ServerId, IpAddress, _instanceName);
            return message;
        }
        ...
        {
            var message = string.Format(ServiceDeskMessageCount, _databaseName, metricTypeDescription, MetricInstanceId, count, metricThreshold.TimePeriod, _minsSinceLast, metricThreshold.Id, metricThreshold.FloorValue, metricThreshold.CeilingValue, Hostname, ServerId, IpAddress, _instanceName);
            return message;
        }
        ...
        {
            var message = string.Format(ServiceDeskMessageAverage, metricTypeDescription, Label, MetricInstanceId, average, metricThreshold.TimePeriod, metricThreshold.Id, metricThreshold.FloorValue, metricThreshold.CeilingValue, Hostname, ServerId, IpAddress, _instanceName);
            return message;
        }
