...
namespace Datavail.Delta.Application.IncidentProcessor.Rules.SqlServerPlugin
{
    public sealed class PerfMetricsCheckpointPagesPerSecRule : IncidentProcessorRule
    {
        ...
        protected override void SetupMatchParams()
        {
            ...
            MatchTypeValue = "Checkpoint Pages/Sec";
        ...
        {
            var message = string.Format(ServiceDeskMessage, MatchTypeValue, MetricInstanceId, _checkpointPagesPerSec, metricThreshold.Id, metricThreshold.FloorValue, metricThreshold.CeilingValue, Hostname, ServerId, IpAddress, _instanceName);
            return message;
        }
        ...
        {
            var message = string.Format(ServiceDeskMessageCount, MatchTypeValue, MetricInstanceId, metricThreshold.NumberOfOccurrences, metricThreshold.TimePeriod, _checkpointPagesPerSec, metricThreshold.Id, metricThreshold.FloorValue, metricThreshold.CeilingValue, Hostname, ServerId, IpAddress, _instanceName);
            return message;
        }
        ...
        {
            var message = string.Format(ServiceDeskMessageAverage, MatchTypeValue, Label, MetricInstanceId, average, metricThreshold.TimePeriod, _checkpointPagesPerSec, metricThreshold.Id, metricThreshold.FloorValue, metricThreshold.CeilingValue, Hostname, ServerId, IpAddress, _instanceName);
            return message;
        }

        protected override void ParseDataCollection(XDocument dataCollection)
        {
            var XCheckpointPagesPerSec = dataCollection.Root.Attribute("checkpointPagesPerSec");
            if (XCheckpointPagesPerSec != null)
            {
                _checkpointPagesPerSec = long.Parse(XCheckpointPagesPerSec.Value);
            }
