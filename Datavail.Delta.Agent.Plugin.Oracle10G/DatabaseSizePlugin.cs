...
{
    public class DatabaseSizePlugin : IPlugin   
    {
        ...
        public DatabaseSizePlugin(IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo oracleServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("DatabaseSizePlugin.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}",
        ...
        private void GetDatabaseInformation()
        {
            ...
            sql.AppendLine("select sum(bytes/1024/1024) \"Database Size in MB\" from dba_segments; ");
            ...
            if (result.FieldCount > 0)
            {
                while (result.Read())
                {
                    var databaseSizeInMb = result["Database Size in MB"].ToString();
    
                    resultCode = "0";
                    resultMessage = "Database Size returned: " + _instanceName;

                    BuildExecuteOutput(databaseSizeInMb, resultCode, resultMessage);
                }

            }
            else
            {
                resultMessage = "Database Size not returned: " + _instanceName;
                BuildExecuteOutput("", resultCode, resultMessage);
            }
        }

        private void BuildExecuteOutput(string databaseSizeInMb, string resultCode, string resultMessage)
        {
            var xml = new XElement("DatabaseSizePluginOutput",
                                   ...
                                   new XAttribute("databaseSizeInMb", databaseSizeInMb)
                                   );
