...
{
    public class TotalReadsWritesByFilesystemPlugin : IPlugin   
    {
        ...
        public TotalReadsWritesByFilesystemPlugin()
        {
        ...
        public TotalReadsWritesByFilesystemPlugin(IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo oracleServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("TotalReadsWritesByFilesystemPlugin.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}",
        ...
        private void GetTopReads()
        {
            ...
            if (result.FieldCount > 0)
            {
                while (result.Read())
                {
                    ...
                    resultCode = "0";
                    resultMessage = "Total Reads Writes By Filesystem returned: " + _instanceName;
                    ...
                    BuildExecuteOutput(file, physicalReads, averageReadTime, physicalWrites, averageWriteTime, resultCode, resultMessage);
                }

            }
            else
            {
                resultMessage = "Total Reads Writes By Filesystem not returned: " + _instanceName;
                BuildExecuteOutput("", "", "", "", "", resultCode, resultMessage);
            }
        }

        private void BuildExecuteOutput(string file, string physicalReads, string averageReadTime, string physicalWrites, 
            string averageWriteTime, string resultCode, string resultMessage)
        {
            var xml = new XElement("TotalReadsWritesByFilesystemPluginOutput",
