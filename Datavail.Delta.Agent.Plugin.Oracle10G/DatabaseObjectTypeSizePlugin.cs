...
{
    public class DatabaseObjectTypeSizePlugin : IPlugin   
    {
        ...
        public DatabaseObjectTypeSizePlugin()
        {
        ...
        public DatabaseObjectTypeSizePlugin(IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo oracleServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("DatabaseObjectTypeSizePlugin.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}",
        ...
        private void GetDatabaseObjectTypeSize()
        {
            ...
            sql.AppendLine("select segment_type \"Object Type\", sum(bytes/1024/1024) as Object_Size from dba_segments group by segment_type; ");
            ...
            if (result.FieldCount > 0)
            {
                while (result.Read())
                {
                    ...
                    var objectSize = result["OBJECT_SIZE"].ToString();
    
                    resultCode = "0";
                    resultMessage = "Database Object Type Size returned: " + _instanceName;
            ...
            {
                resultMessage = "Database Object Type Size not returned: " + _instanceName;
                BuildExecuteOutput("", "", resultCode, resultMessage);
            }
        }

        private void BuildExecuteOutput(string objectType, string objectSize, string resultCode, string resultMessage)
        {
            var xml = new XElement("DatabaseObjectTypeSizePluginOutput",
                                   ...
                                   new XAttribute("objectSize", objectSize)
                                   );
