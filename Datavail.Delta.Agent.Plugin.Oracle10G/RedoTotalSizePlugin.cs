...
{
    public class RedoTotalSizePlugin : IPlugin   
    {
        ...
        public RedoTotalSizePlugin(IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo oracleServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("RedoTotalSizePlugin.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}",
        ...
        private void GetInvalidObjects()
        {
            ...
            if (result.FieldCount > 0)
            {
                while (result.Read())
                {
                    var firstTime = result["FIRSTTIME"].ToString();
                    var gBytes = result["GBYTES"].ToString();
                    var count = result["CNT"].ToString();
                    var averagePerHour = result["AVG_PER_HR"].ToString();

                    resultCode = "0";
                    resultMessage = "Redo Total Size returned: " + _instanceName;

                    BuildExecuteOutput(firstTime, gBytes, count, averagePerHour, resultCode, resultMessage);
                }

            }
            else
            {
                resultMessage = "Redo Total Size not returned: " + _instanceName;
                BuildExecuteOutput("", "", "", "", resultCode, resultMessage);
            }
        }

        private void BuildExecuteOutput(string firstTime, string gBytes, string count, string averagePerHour, string resultCode, string resultMessage)
        {
            var xml = new XElement("RedoTotalSizePluginOutput",
                                   ...
                                   new XAttribute("firstTime", firstTime),
                                   new XAttribute("gBytes", gBytes),
                                   new XAttribute("count", count),
                                   new XAttribute("averagePerHour", averagePerHour)
                                   );
