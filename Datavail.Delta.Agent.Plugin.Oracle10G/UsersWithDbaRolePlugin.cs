...
{
    public class UsersWithDbaRolePlugin : IPlugin   
    {
        ...
        public UsersWithDbaRolePlugin(ICommon common, IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo oracleServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("UsersWtihDbaRolePlugin.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}",
        ...
        private void GetInvalidObjects()
        {
            ...
            if (result.FieldCount > 0)
            {
                while (result.Read())
                {
                    ...
                    resultCode = "0";
                    resultMessage = "Users Wtih Dba Role returned: " + _instanceName;

                    BuildExecuteOutput(userName, resultCode, resultMessage);
                }

            }
            else
            {
                resultMessage = "Users Wtih Dba Role not returned: " + _instanceName;
                BuildExecuteOutput("", resultCode, resultMessage);
            }
        }

        private void BuildExecuteOutput(string userName, string resultCode, string resultMessage)
        {
            var xml = new XElement("UsersWtihDbaRolePluginOutput",
