...
{
    public class InstanceStatusPlugin : IPlugin   
    {
        ...
        public InstanceStatusPlugin(IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo oracleServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("InstanceStatusPlugin.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}",
        ...
        private void GetInstanceStatus()
        {
            ...
            if (result.FieldCount > 0)
            {
                ...
                resultCode = "0";
                resultMessage = "Instance Status returned for Instance: " + _instanceName;

                BuildExecuteOutput("Up", resultCode, resultMessage);
                
            }
            else
            {
                resultMessage = "Instance Status not returned: " + _instanceName;
                BuildExecuteOutput("Down", resultCode, resultMessage);
            }
        }

        private void BuildExecuteOutput(string instanceStatus, string resultCode, string resultMessage)
        {
            var xml = new XElement("InstanceStatusPluginOutput",
                                   ...
                                   new XAttribute("instanceStatus", instanceStatus));
