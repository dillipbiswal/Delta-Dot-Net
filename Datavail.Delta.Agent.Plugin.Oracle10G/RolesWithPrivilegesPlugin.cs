...
{
    public class RolesWithPrivilegesPlugin : IPlugin   
    {
        ...
        public RolesWithPrivilegesPlugin()
        {
        ...
        public RolesWithPrivilegesPlugin(IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo oracleServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("RolesWithPrivilegesPlugin.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}",
        ...
        private void GetUserRolesGranted()
        {
            ...
            sql.AppendLine("select GRANTEE,PRIVILEGE,ADMIN_OPTION ");
            sql.AppendLine("from  dba_sys_Privs ");
            sql.AppendLine("order by grantee, PRIVILEGE; ");
            ...
            if (result.FieldCount > 0)
            {
                while (result.Read())
                {
                    ...
                    resultCode = "0";
                    resultMessage = "Roles With Privileges returned: " + _instanceName;

                    var grantee = result["GRANTEE"].ToString();
                    var privilege = result["PRIVILEGE"].ToString();
                    var adminOption = result["ADMIN_OPTION"].ToString();

                    BuildExecuteOutput(grantee, privilege, adminOption, resultCode, resultMessage);
                }

            }
            else
            {
                resultMessage = "Roles With Privileges not returned: " + _instanceName;
                BuildExecuteOutput("", "", "", resultCode, resultMessage);
            }
        }

        private void BuildExecuteOutput(string grantee, string privilege, string adminOption, string resultCode, string resultMessage)
        {
            var xml = new XElement("RolesWithPrivilegesGrantedPluginOutput",
                                   ...
                                   new XAttribute("privilege", privilege),
                                   new XAttribute("adminOption", adminOption)
                                   );
