...
{
    public class NonDefaultInitParametersPlugin : IPlugin   
    {
        ...
        public NonDefaultInitParametersPlugin()
        {
        ...
        public NonDefaultInitParametersPlugin(IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo oracleServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("NonDefaultInitParametersPlugin.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}",
        ...
        private void GetNonDefaultParams()
        {
            var resultCode = "-1";
            ...
            sql.AppendLine("SELECT  name init_parameter_name, value init_parameter_value FROM v$parameter WHERE isdefault = 'FALSE' ORDER BY name");
            ...
            if (result.FieldCount > 0)
            {
                while (result.Read())
                {
                    var name = result["INIT_PARAMETER_NAME"].ToString();
                    var value = result["INIT_PARAMETER_VALUE"].ToString();
    
                    resultCode = "0";
                    resultMessage = "Non Default Init Parameters returned: " + _instanceName;

                    BuildExecuteOutput(name, value, resultCode, resultMessage);
                }

            }
            else
            {
                resultMessage = "Non Default Init Parameters not returned: " + _instanceName;
                BuildExecuteOutput("", "", resultCode, resultMessage);
            }
        }

        private void BuildExecuteOutput(string name, string value, string resultCode, string resultMessage)
        {
            var xml = new XElement("NonDefaultInitParametersPluginOutput",
