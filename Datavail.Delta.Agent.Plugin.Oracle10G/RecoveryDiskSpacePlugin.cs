...
{
    public class RecoveryDiskSpacePlugin : IPlugin   
    {
        ...
        public RecoveryDiskSpacePlugin()
        {
        ...
        public RecoveryDiskSpacePlugin(IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo oracleServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("RecoveryDiskSpace.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}",
        ...
        private void GetInstanceStatus()
        {
            ...
            sql.Append("select (SPACE_USED/SPACE_LIMIT)*100as spaceused from v$recovery_file_dest");
            ...
            if (result.FieldCount > 0)
            {
                while (result.Read())
                {
                    var spaceUsed = result["SPACEUSED"].ToString();

                    resultCode = "0";
                    resultMessage = "Recovery Space Used returned for Instance: " + _instanceName;

                    BuildExecuteOutput(spaceUsed, resultCode, resultMessage);

                }   
            }
            else
            {
                resultMessage = "Recovery Space Used not returned: " + _instanceName;
                BuildExecuteOutput("n/a", resultCode, resultMessage);
            }
        }

        private void BuildExecuteOutput(string spaceUsed, string resultCode, string resultMessage)
        {
            var xml = new XElement("RecoveryDiskSpacePluginOutput",
                                   ...
                                   new XAttribute("spaceUsed", spaceUsed));
