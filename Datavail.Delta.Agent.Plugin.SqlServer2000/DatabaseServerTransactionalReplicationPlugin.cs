    ...
    public class DatabaseServerTransactionalReplicationPlugin : IPlugin
    {
        ...
        public void Execute(Guid metricInstance, string label, string data)
        {
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    _output = _logger.BuildErrorOutput("SqlServer2000.DatabaseServerTransactionalReplicationPlugin", "Execute", _metricInstance, ex.ToString());
                    _dataQueuer.Queue(_output);
                }
                catch { }
            }
        ...
        private void GetTransactionalReplicationStatus()
        {
            ...
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                while (publishers.Read())
                {
                    ...
                    {
                        var result = SqlHelper.GetDataReader(conn1, sbSql.ToString());
                        ...
                        if (result.FieldCount > 0)
                        {
                            while (result.Read())
                            {
                                ...
                                xml.Root.Add(BuildExecuteOutputNode(subscriber, publication2, status, subscriberDb, type, distributionAgent, lastAction, startTime,
                                    actionTime, duration, deliveryLatency, deliveryTransactions, deliveredCommands,
                                    averageCommands, errorId, jobId, localJob, profileId, agentId, offloadEnabled,
                        ...
                        if (resultCount == 0)
                        {
                            ...
                            xml.Root.Add(BuildExecuteOutputNode("n/a", "n/a", "n/a", "n.a", "n/a", "n/a", "n/a", "n/a", "n/a", "n/a", "n/a",
                                "n/a", "n/a", "n/a", "n/a", "n/a", "n/a", "n/a", "n/a", "n/a", "n/a", "n/a", resultCode, resultMessage));
                        }
                        conn1.Dispose();
                        conn1.Close();
                    }
