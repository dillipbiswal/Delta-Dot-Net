...
namespace Datavail.Delta.Application.Tests.IncidentProcessor.Rules.HostPlugin_Tests.PhysicalMemoryThresholdRule_Tests
{
    ...
    public class WhenMatchingWithSingleCountPercentageThreshold
    {
        ...
        public void ThenServiceDeskMessageIsCorrect()
        {
            ...
            var xml = GetMatchingXml();
            var rule = new PhysicalMemoryThresholdRule(incidentService.Object, xml, serverService);
            var expectedMessage = string.Format("The Delta monitoring application has detected a physical memory {0} threshold breach for {1} (metricInstanceId: {2}).\n\nTotal Bytes: {3} ({4:N0})\nAvailable Bytes: {5} ({6:N0})\nPercentage Available: {7:0.00}%\n\nMetric Threshold: {8}\nFloor Value: {9:N2}\nCeiling Value: {10:N2}\nServer: {11} ({12})\nIp Address: {13}\n", MatchType, Label, MetricInstanceId, TotalPhysicalMemoryBytesFriendly, TotalPhysicalMemoryBytes, AvailablePhysicalMemoryFriendly, AvailablePhysicalMemoryBytes, PercentagePhysicalMemoryAvailable, _metricThresholdId.ToString(), Floor, Ceiling, Hostname, ServerId, IpAddress);
