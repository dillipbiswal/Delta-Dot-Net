...
namespace Datavail.Delta.Application.Tests.IncidentProcessor.Rules.HostPlugin_Tests.PhysicalMemoryThresholdRule_Tests
{
    ...
    public class WhenMatchingWithMultipleCountPercentageThreshold
    {
        ...
        public void ThenServiceDeskMessageIsCorrect()
        {
            ...
            var xml = GetMatchingXml();
            var rule = new PhysicalMemoryThresholdRule(incidentService.Object, xml, serverService);
            var expectedMessage = string.Format("The Delta monitoring application has detected a physical memory {0} threshold breach for {1} (metricInstanceId: {2}). This has occurred {3} times in the last {4} minutes.\n\nTotal Bytes: {5} ({6:N0})\nAvailable Bytes: {7} ({8:N0})\nPercentage Available: {9:0.00}%\n\nMetric Threshold: {10}\nFloor Value: {11:N2}\nCeiling Value: {12:N2}\nServer: {13} ({14})\nIp Address: {15}\n", MatchType, Label, MetricInstanceId, MatchCount, TimePeriod, TotalPhysicalMemoryBytesFriendly, TotalPhysicalMemoryBytes, AvailablePhysicalMemoryFriendly, AvailablePhysicalMemoryBytes, PercentageAvailable, _metricThresholdId.ToString(), Floor, Ceiling, Hostname, ServerId, IpAddress);
