...
namespace Datavail.Delta.IncidentProcessor
{
    ...
    public class InventoryProcessorWorker : WorkerBase
    {
        ...
        private DataCollectionInventoryMessage _message;
        private static int processedCount = 0;


        public InventoryProcessorWorker(IKernel kernel, IDeltaLogger logger, IQueue<DataCollectionInventoryMessage> inventoryQueue, IQueue<OpenIncidentMessage> openIncidentQueue, IQueue<DataCollectionMessageWithError> errorQueue)
        {
            _kernel = kernel;
            _logger = logger;
                ...
                while (ServiceStarted)
                {
                    try
                    {
                        if (DateTime.Now.Second == 0 && lastMinuteProcessed != DateTime.Now.Minute)
                        {
                            lock (theLock)
                            {
                                _logger.LogInformational(WellKnownErrorMessages.InformationalMessage, "Inventory Processor processed " + processedCount + " in the last minute");
                                Trace.WriteLine("Inventory Processor processed " + processedCount + " in the last minute");
                                processedCount = 0;
                                lastMinuteProcessed = DateTime.Now.Minute;
                            }
                        }

                        _message = _inventoryQueue.GetMessage();
                        ...
                        if (_message != null)
                        {
                            ...
                            if (string.IsNullOrEmpty(_message.Data))
                            {
                                ...
                                _inventoryQueue.DeleteMessage(_message);
                                continue;
                            }
