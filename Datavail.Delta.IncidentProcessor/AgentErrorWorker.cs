...
{
    public class AgentErrorWorker : WorkerBase
    {
        ...
        private DataCollectionAgentErrorMessage _message;
        private static int processedCount = 0;


        public AgentErrorWorker(IKernel kernel, IDeltaLogger logger, IQueue<DataCollectionAgentErrorMessage> agentErrorQueue, IQueue<OpenIncidentMessage> openIncidentQueue, IQueue<DataCollectionMessageWithError> errorQueue)
        {
            _kernel = kernel;
            _logger = logger;
                ...
                while (ServiceStarted)
                {
                    try
                    {
                        if (DateTime.Now.Second == 0 && lastMinuteProcessed != DateTime.Now.Minute)
                        {
                            lock (theLock)
                            {
                                _logger.LogInformational(WellKnownErrorMessages.InformationalMessage, "Agent Error processed " + processedCount + " in the last minute");
                                Trace.WriteLine("Agent Error processed " + processedCount + " in the last minute");
                                processedCount = 0;
                                lastMinuteProcessed = DateTime.Now.Minute;
                            }
                        }

                        _message = _agentErrorQueue.GetMessage();
                        ...
                        if (_message != null)
                        {
                            ...
                            if (string.IsNullOrEmpty(_message.Data))
                            {
                                ...
                                _agentErrorQueue.DeleteMessage(_message);
                                continue;
                            }
