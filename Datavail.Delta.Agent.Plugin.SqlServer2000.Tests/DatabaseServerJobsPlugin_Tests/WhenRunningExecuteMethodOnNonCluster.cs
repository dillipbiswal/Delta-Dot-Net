...
namespace Datavail.Delta.Agent.Plugin.SqlServer2000.Tests.DatabaseServerJobsPlugin_Tests
{
    ...
    public class WhenRunningExecuteMethodOnNonCluster
    {
        ...
        [Ignore]
        [TestMethod]
        public void ThenNoJobStatusFoundIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            var plugin = new DatabaseServerJobsPlugin(clusterInfo.Object, dataUploader.Object, logger.Object, sqlRunner, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("DatabaseServerJobsPluginInput", new XAttribute("InstanceName", MockInstanceName), new XAttribute("ConnectionString", connectionString), new XAttribute("JobName", JobName));
            ...
            Assert.IsTrue(uploadedData.Contains("resultMessage=\"" + JobName + " not found." + "\""));
