...
ko.utils.domNodeDisposal = new (function () {
    ...
    function cleanSingleNode(node) {
        ...
        // Also erase the DOM Contents
        ko.utils.domData.clear(node);

        // Special support for jQuery here because it's so commonly used.
        // Many jQuery plugins (including jquery.tmpl) store Contents using jQuery's equivalent of domData
...
(function () {
    ...
    function simpleHtmlParse(html) {
        ...
        // Finds the first match from the left column, and returns the corresponding "wrap" Contents from the right column
...
(function() {
    var defaultBindingAttributeName = "Contents-bind";
...
(function () {
    ...
    function applyBindingsToNodeAndDescendantsInternal (viewModel, nodeVerified, bindingContextMayDifferFromDomParentElement) {
        ...
        // (2) It might have bindings (e.g., it has a Contents-bind attribute, or it's a marker for a containerless template)
    ...
    function applyBindingsToNodeInternal (node, bindings, viewModelOrBindingContext, bindingContextMayDifferFromDomParentElement) {
        ...
        // Each time the dependentObservable is evaluated (after Contents changes),
        // the binding attribute is reparsed so that it can pick out the correct
        // model properties in the context of the changed Contents.
        // DOM event callbacks need to be able to access this changed Contents,
    ...
    ko.contextFor = function(node) {
        // We can only do something meaningful for elements and comment nodes (in particular, not text nodes, as IE can't store domContents for them)
        switch (node.nodeType) {
            case 1:
            case 8:
    ...
    ko.exportSymbol('ContentsFor', ko.dataFor);
})();
// For certain common events (currently just 'click'), allow a simplified Contents-binding syntax
        ...
        if (value) {
            ...
            for (var i = 0, j = value.length; i < j; i++) {
                ...
                if (typeof optionsTextValue == "function")
                    optionText = optionsTextValue(value[i]); // Given a function; run it against the Contents value
                else if (typeof optionsTextValue == "string")
                    optionText = value[i][optionsTextValue]; // Given a string; treat it as a property name on the Contents value
                else
                    optionText = optionValue;				 // Given no optionsText arg; use the Contents value itself
...
// "with: someExpression" is equivalent to "template: { if: someExpression, Contents: someExpression }"
...
// "foreach: { Contents: someExpression, afterAdd: myfn }" is equivalent to "template: { foreach: someExpression, afterAdd: myfn }"
ko.bindingHandlers['foreach'] = {
    makeTemplateValueAccessor: function(valueAccessor) {
        return function() {
            ...
            // If bindingValue.Contents is the array, preserve all relevant options
            return {
...
//            // - bindingContext.$Contents is the Contents you should pass into the template
//            //   - you might also want to make bindingContext.$parent, bindingContext.$parents,
//            //     and bindingContext.$root available in the template too
//            // - options gives you access to any other properties set on "Contents-bind: { template: options }"
//            //
...
//        }
//
//     This is only necessary if you want to allow Contents-bind attributes to reference arbitrary template variables.
...
ko.templateRewriting = (function () {
    var memoizeDataBindingAttributeSyntaxRegex = /(<[a-z]+\d*(\s+(?!Contents-bind=)[a-z0-9\-]+(=(\"[^\"]*\"|\'[^\']*\'))?)*\s+)Contents-bind=(["'])([\s\S]*?)\5/gi;
    ...
    return {
        ...
        memoizeBindingAttributeSyntax: function (htmlString, templateEngine) {
            return htmlString.replace(memoizeDataBindingAttributeSyntaxRegex, function () {
                return constructMemoizedTagReplacement(/* ContentsBindAttributeValue: */ arguments[6], /* tagToRetain: */ arguments[1], templateEngine);
            }).replace(memoizeVirtualContainerBindingSyntaxRegex, function() {
                return constructMemoizedTagReplacement(/* ContentsBindAttributeValue: */ arguments[1], /* tagToRetain: */ "<!-- ko -->", templateEngine);
            });
        },
...
(function() {
    ...
    //   Contents(key)			- reads values stored using Contents(key, value) - see below
    //   Contents(key, value)	- associates "value" with this template and the key "key". Is used to store information like "isRewritten".
    //
    ...
    // Writing to "text" is still supported, but then the template Contents will not be available as DOM nodes.
        ...
        if (targetNodeOrNodeArray) {
            ...
            return ko.dependentObservable( // So the DOM is automatically updated when any dependency changes
                function () {
                    ...
                    // Support selecting template as a function of the Contents being rendered
    ...
    ko.bindingHandlers['template'] = {
        ...
        'update': function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            if ((typeof bindingValue === 'object') && ('foreach' in bindingValue)) { // Note: can't use 'in' operator on strings
                // Render once for each Contents point (treating Contents set as empty if shouldDisplay==false)
                ...
                if (shouldDisplay) {
                    // Render once for this single Contents point (or use the viewModel if no Contents was provided)
                    var innerBindingContext = (typeof bindingValue == 'object') && ('Contents' in bindingValue)
                        ? bindingContext['createChildContext'](ko.utils.unwrapObservable(bindingValue['data'])) // Given an explitit 'Contents' value, we create a child binding context for it
                        : bindingContext;                                                                       // Given no explicit 'Contents' value, we retain the same binding context
...
(function() {
    ko.jqueryTmplTemplateEngine = function () {
        ...
        this['renderTemplateSource'] = function(templateSource, bindingContext, options) {
            ...
            var data = [bindingContext['$data']]; // Prewrap the Contents in an array to stop jquery.tmpl from trying to unwrap any arrays
