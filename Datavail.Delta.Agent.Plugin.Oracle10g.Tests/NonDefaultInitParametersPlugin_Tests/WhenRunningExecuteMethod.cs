...
namespace Datavail.Delta.Agent.Plugin.Oracle10g.Tests.NonDefaultInitParametersPluginInputPlugin_Tests
{
    [TestClass]
    public class WhenRunningExecuteMethod
    {
        ...
        private static IDataReader SetupMockDataReader()
        {
            ...
            dataReader.Setup(r => r["INIT_PARAMETER_NAME"]).Returns("audit_file_dest");
            dataReader.Setup(r => r["INIT_PARAMETER_VALUE"]).Returns("/opt/apps/oracle/product/db10204/admin/test10g/adump");
        ...
        public void ThenDataIsUploaded()
        {
            ...
            var plugin = new NonDefaultInitParametersPlugin(dataUploader.Object, logger.Object,
                                                  sqlRunner, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("NonDefaultInitParametersPluginInput",
        ...
        [TestMethod]
        public void ThenNameUsedIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("name=\"audit_file_dest"));
        }
        ...
        public void ThenValueIsUploaded()
        {
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("value=\"/opt/apps/oracle/product/db10204/admin/test10g/adump"));
        }
        ...
        public void ThenProductIsPopulatedInUploadedData()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new NonDefaultInitParametersPlugin(dataUploader.Object, logger.Object, sqlRunner, sqlServerInfo);
        ...
        public void ThenLogEntryIsWrittenWhenExceptionIsThrown()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new NonDefaultInitParametersPlugin(dataUploader.Object, logger.Object, sqlRunner.Object, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("NonDefaultInitParametersPluginInput",
                                         new XAttribute("ConnectionString", connectionString),
                                         new XAttribute("InstanceName", MockInstanceName));
