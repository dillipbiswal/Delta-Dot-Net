...
namespace Datavail.Delta.Agent.Plugin.Oracle10g.Tests.SgaInformationPlugin_Tests
{
    [TestClass]
    public class WhenRunningExecuteMethod
    {
        ...
        private static IDataReader SetupMockDataReader()
        {
            ...
            dataReader.Setup(r => r["Name"]).Returns("Database Buffers");
            dataReader.Setup(r => r["Size in MB"]).Returns("25.75");
        ...
        public void ThenDataIsUploaded()
        {
            ...
            var plugin = new SgaInformationPlugin(dataUploader.Object, logger.Object,
                                                  sqlRunner, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("SgaInformationPluginInput",
        ...
        public void ThenNameUsedIsUploaded()
        {
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("name=\"Database Buffers"));
        }

        [TestMethod]
        public void ThenSizeInMbIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("sizeInMb=\"25.75"));
        }
        ...
        public void ThenProductIsPopulatedInUploadedData()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new SgaInformationPlugin(dataUploader.Object, logger.Object, sqlRunner, sqlServerInfo);
        ...
        public void ThenLogEntryIsWrittenWhenExceptionIsThrown()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new SgaInformationPlugin(dataUploader.Object, logger.Object, sqlRunner.Object, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("SgaInformationPluginInput",
                                         new XAttribute("ConnectionString", connectionString),
                                         new XAttribute("InstanceName", MockInstanceName));
