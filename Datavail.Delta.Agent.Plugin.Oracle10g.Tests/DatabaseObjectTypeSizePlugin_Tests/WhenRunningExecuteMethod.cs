...
namespace Datavail.Delta.Agent.Plugin.Oracle10g.Tests.DatabaseOBjectTypeSizePlugin_Tests
{
    [TestClass]
    public class WhenRunningExecuteMethod
    {
        ...
        private static IDataReader SetupMockDataReader()
        {
            ...
            dataReader.Setup(r => r["OBJECT_TYPE"]).Returns("NESTED TABLE");
            dataReader.Setup(r => r["OBJECT_SIZE"]).Returns(".6875");
        ...
        public void ThenDataIsUploaded()
        {
            ...
            var plugin = new DatabaseObjectTypeSizePlugin(dataUploader.Object, logger.Object,
                                                  sqlRunner, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("DatabaseObjectTypeSizePluginInput",
        ...
        public void ThenObjectTypeIsUploaded()
        {
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("objectType=\"NESTED TABLE"));
        }

        [TestMethod]
        public void ThenObjectSizeIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("objectSize=\".6875"));
        }
        ...
        public void ThenProductIsPopulatedInUploadedData()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new DatabaseObjectTypeSizePlugin(dataUploader.Object, logger.Object, sqlRunner, sqlServerInfo);
        ...
        public void ThenLogEntryIsWrittenWhenExceptionIsThrown()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new DatabaseObjectTypeSizePlugin(dataUploader.Object, logger.Object, sqlRunner.Object, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("DatabaseObjectTypeSizePluginInput",
                                         new XAttribute("ConnectionString", connectionString),
                                         new XAttribute("InstanceName", MockInstanceName));
