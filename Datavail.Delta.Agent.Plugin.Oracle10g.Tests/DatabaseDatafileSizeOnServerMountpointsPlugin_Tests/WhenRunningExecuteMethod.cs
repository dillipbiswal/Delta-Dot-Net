...
namespace Datavail.Delta.Agent.Plugin.Oracle10g.Tests.DatabaseDatafileSizeOnServerMountpointsPlugin_Tests
{
    [TestClass]
    public class WhenRunningExecuteMethod
    {
        ...
        private static IDataReader SetupMockDataReader()
        {
            ...
            dataReader.Setup(r => r["File"]).Returns("/dat");
            dataReader.Setup(r => r["SUM"]).Returns("8575");
        ...
        public void ThenDataIsUploaded()
        {
            ...
            var plugin = new DatabaseDatafileSizeOnServerMountpointsPlugin(dataUploader.Object, logger.Object,
                                                  sqlRunner, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("DatabaseDatafileSizeOnServerMountpointsPluginInput",
        ...
        public void ThenFileIsUploaded()
        {
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("file=\"/dat"));
        }

        [TestMethod]
        public void ThenSumIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("sum=\"8575"));
        }
        ...
        public void ThenProductIsPopulatedInUploadedData()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new DatabaseDatafileSizeOnServerMountpointsPlugin(dataUploader.Object, logger.Object, sqlRunner, sqlServerInfo);
        ...
        public void ThenLogEntryIsWrittenWhenExceptionIsThrown()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new DatabaseDatafileSizeOnServerMountpointsPlugin(dataUploader.Object, logger.Object, sqlRunner.Object, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("DatabaseDatafileSizeOnServerMountpointsPluginInput",
                                         new XAttribute("ConnectionString", connectionString),
                                         new XAttribute("InstanceName", MockInstanceName));
