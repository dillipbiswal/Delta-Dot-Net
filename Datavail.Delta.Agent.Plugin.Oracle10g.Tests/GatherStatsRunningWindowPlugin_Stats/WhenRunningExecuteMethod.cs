...
namespace Datavail.Delta.Agent.Plugin.Oracle10g.Tests.GatherStatsRunningWindowPlugin_Tests
{
    [TestClass]
    public class WhenRunningExecuteMethod
    {
        ...
        private static IDataReader SetupMockDataReader()
        {
            ...
            dataReader.Setup(r => r["JOB_NAME"]).Returns("Gather Stats Job");
            dataReader.Setup(r => r["Running"]).Returns("TRUE");
            dataReader.Setup(r => r["WINDOW_NAME"]).Returns("WEEKEND_WINDOW");
            dataReader.Setup(r => r["SCHEDULE_NAME"]).Returns("Weekend Window");
            dataReader.Setup(r => r["START_DATE"]).Returns("04-OCT-2011 22:00:02");
            dataReader.Setup(r => r["REPEAT_INTERVAL"]).Returns("freq=daily;byday=SAT;byhour=0;byminute=0;bysecond=0");
        ...
        public void ThenDataIsUploaded()
        {
            ...
            var plugin = new GatherStatsRunningWindowPlugin(dataUploader.Object, logger.Object,
                                                  sqlRunner, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("GatherStatsRunningWindowPluginInput",
        ...
        [TestMethod]
        public void ThenJobNameIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("jobName=\"Gather Stats Job"));
        }

        [TestMethod]
        public void ThenRunningIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("running=\"TRUE"));
        }

        [TestMethod]
        public void ThenWindowNameIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("windowName=\"WEEKEND_WINDOW"));
        }

        [TestMethod]
        public void ThenScheduleNameIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("scheduleName=\"Weekend Window"));
        }

        [TestMethod]
        public void ThenStartDateIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("startDate=\"04-OCT-2011 22:00:02"));
        }

        [TestMethod]
        public void ThenRepeatIntervalIsUploaded()
        {
            //Arrange
            var metricInstanceId = Guid.NewGuid();
            ...
            //Assert
            Assert.IsTrue(uploadedData.Contains("repeatInterval=\"freq=daily;byday=SAT;byhour=0;byminute=0;bysecond=0"));
        }
        ...
        public void ThenProductIsPopulatedInUploadedData()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new GatherStatsRunningWindowPlugin(dataUploader.Object, logger.Object, sqlRunner, sqlServerInfo);
        ...
        public void ThenLogEntryIsWrittenWhenExceptionIsThrown()
        {
            ...
            var logger = new Mock<IDeltaLogger>();
            var sqlServerInfo = SetupMockSqlServerInfo();
            var plugin = new GatherStatsRunningWindowPlugin(dataUploader.Object, logger.Object, sqlRunner.Object, sqlServerInfo);
            var connectionString = SetupMockConnectionString();
            var inputData = new XElement("GatherStatsRunningWindowPluginInput",
                                         new XAttribute("ConnectionString", connectionString),
                                         new XAttribute("InstanceName", MockInstanceName));
