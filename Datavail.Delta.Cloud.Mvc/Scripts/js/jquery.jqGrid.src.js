...
(function ($) {
    ...
    $.fn.jqGrid = function (pin) {
        ...
        setPager = function (pgid, tp) {
            ...
            $("#" + $.jgrid.jqID(pgid))
			.append("<div id='" + pgcnt + "' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:auto;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='" + lft + "' align='left'></td><td id='" + cent + "' align='center' style='white-space:pre;'></td><td id='" + rgt + "' align='right'></td></tr></tbody></table></div>")
...
//		.attr({tabIndex: "-1","role":"dialog","aria-labelledby":aIDs.modalhead,"aria-hidden":"true"});
	...
	viewModal : function (selector,o){
		...
		if ($.fn.jqm && o.jqm === true) {
            ...
            if ($(selector).dialog('isOpen') !== false){

               $(selector).attr("aria-hidden","false").dialog( {modal: true,
                                width: o.width,
                                draggable: false,
                                resizable: false,
                                title: $(selector).data("title"),
                                show: { effect: 'slide', complete: function() { setTimeout(function () {
                                    try {$(':input:visible',selector)[0].focus();} catch (e) {}
                                }, 50) }}, 
                                hide: { effect: "fade", duration: 300 },
                                close: function(event, ui){
                                    $.jgrid.hideModal(selector,{gb:o.gbox,jqm:false,onClose: $(selector).data("onClose")});
                                },
                                buttons:
                                        { 
                                            "Confirm": {
                                            ...
                                            "Cancel": {
                                                        id: o.leftButtonId,
                                                        text: o.leftButton,
                                                        click: function () {$(selector).dialog("close");}
                                            }
                                        }
                });
               ...
               $(selector).dialog('option', 'title', $(selector).data("title")); 
		...
		return this.each(function(){
			...
			function postIt() {
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					$("#sData", $("div[aria-labelledby='ui-dialog-title-" + IDs.themodal + "']")).addClass('ui-state-active');
					oper = opers.oper;
					idname = opers.id;
					...
					var ajaxOptions = $.extend({
						...
						complete:function(data,Status){
							...
							$("#sData", $("div[aria-labelledby='ui-dialog-title-" + IDs.themodal + "']")).removeClass('ui-state-active');
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
                ...
                $("#" + IDs.themodal).data("title", p.caption);
				$.jgrid.viewModal("#"+IDs.themodal,{gbox:"#gbox_"+gID,jqm:p.jqModal, jqM: false, overlay: p.overlay, modal:p.modal, width: p.width});
				if(!closeovrl) {
                ...
                // buttons at footer
				var bP = "<a href='javascript:void(0)' id='"+bp+"' class='ui-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>",
				bN = "<a href='javascript:void(0)' id='"+bn+"' class='ui-button ui-state-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>",
				bS  ="<a href='javascript:void(0)' id='sData' class='ui-button ui-state-default ui-corner-all'>"+p.bSubmit+"</a>",
				bC  ="<a href='javascript:void(0)' id='cData' class='ui-button ui-state-default ui-corner-all'>"+p.bCancel+"</a>";
				...
				$("#"+IDs.themodal).keydown( function( e ) {
					...
					if(rp_ge[$t.p.id].savekey[0] === true && e.which == rp_ge[$t.p.id].savekey[1]) { // save
						if(wkey.tagName != "TEXTAREA") {
							$("#sData", $("div[aria-labelledby='ui-dialog-title-" + IDs.themodal + "']")).trigger("click");
							return false;
						}
					}
					if(e.which === 27) {
				...
				if(p.saveicon[0]===true) {
					$("#sData",$("div[aria-labelledby='ui-dialog-title-" + IDs.themodal + "']")).addClass(p.saveicon[1] == "right" ? 'fm-button-icon-right' : 'fm-button-icon-left')
				...
				if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
					bS  ="<a href='javascript:void(0)' id='sNew' class='ui-button ui-state-default ui-corner-all' style='z-index:1002'>"+p.bYes+"</a>";
					bN  ="<a href='javascript:void(0)' id='nNew' class='ui-button ui-state-default ui-corner-all' style='z-index:1002'>"+p.bNo+"</a>";
					bC  ="<a href='javascript:void(0)' id='cNew' class='ui-button ui-state-default ui-corner-all' style='z-index:1002'>"+p.bExit+"</a>";
				...
				if(onBeforeShow) {onBeforeShow($("#"+frmgr));}
				$("#"+IDs.themodal).data("onClose",rp_ge[$t.p.id].onClose);
				$.jgrid.viewModal("#"+IDs.themodal,{gbox:"#gbox_"+gID,jqm:p.jqModal, overlay: p.overlay,modal:p.modal, width: p.width});
				if(!closeovrl) {
					$(".jqmOverlay").click(function(){
						if(!checkUpdates()) {return false;}
						$.jgrid.hideModal("#"+IDs.themodal,{gb:"#gbox_"+gID,jqm:p.jqModal, onClose: rp_ge[$t.p.id].onClose});
				...
				$("#sData", $("div[aria-labelledby='ui-dialog-title-" + IDs.themodal + "']")).click(function(e){
					postdata = {};extpost={};
					$("#FormError","#"+frmtb).hide();
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
                ...
                $("#"+IDs.themodal).data("title", p.caption);
				$.jgrid.viewModal("#"+IDs.themodal,{gbox:"#gbox_"+gID,
				...
				var tbl = "<div id='"+dtbl+"' class='formdata' style='width:100%;position:relative;height:"+dh+";'>";
...
//				}
				...
				if(onBeforeShow) {rp_ge[$t.p.id].beforeShowForm($("#"+dtbl));}
                $("#"+IDs.themodal).data("title", p.caption);
				$.jgrid.viewModal("#"+IDs.themodal,{gbox:"#gbox_"+gID,jqm:rp_ge[$t.p.id].jqModal, 
                                                    overlay: rp_ge[$t.p.id].overlay, 
                                                    modal:rp_ge[$t.p.id].modal,
                                                    width: p.width, 
                                                    rightButton: "Delete", 
                                                    ...
                                                    leftButton: "Cancel", 
                                                    leftButtonId: "eData"});
                $("#dData", $("div[aria-labelledby='ui-dialog-title-" + IDs.themodal + "']")).click(function(e){
					var ret=[true,""];onCS = {};
				...
				$("#eData", $("div[aria-labelledby='ui-dialog-title-" + IDs.themodal + "']")).click(function(e){
    ...
    SearchValue : function(colname, colvalue){
        return this.each(function() {
            var $t = this;
            var sdata = {searchField: colname, searchString: colvalue, searchOper: "eq", filters: ""  };
