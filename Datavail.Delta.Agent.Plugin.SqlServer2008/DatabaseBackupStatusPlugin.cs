        ...
        private void GetBackupStatus()
        {
            ...
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                if (result.Read())
                {
                    ...
                    BuildExecuteOutput(_databaseName, physicalDeviceName, backupFinishTimeStamp.ToString(),
                        minsSinceLast, resultCode, resultMessage);
                }
                else
                {
                    resultMessage = "No backup history found for database: " + _databaseName;
                    BuildExecuteOutput(_databaseName, "N/A", DateTime.MinValue.ToString(), "-1", resultCode,
                        resultMessage);
                }
            }
        }

        private void BuildExecuteOutput(string databaseName, string physicalDeviceName, string backupFinishDateTime, string minsSinceLast, string resultCode, string resultMessage)
        {
            var xml = new XElement("DatabaseBackupStatusPluginOutput",
                                   ...
                                   new XAttribute("backupFinishTimeStamp", backupFinishDateTime),
