...
namespace Datavail.Delta.Agent.Plugin.SqlServer2008
{
    ...
    public class DatabaseInventoryPlugin : IPlugin
    {
        ...
        public DatabaseInventoryPlugin()
        {
        ...
        public DatabaseInventoryPlugin(IClusterInfo clusterInfo, IDataQueuer dataQueuer, IDeltaLogger logger, ISqlRunner sqlRunner, IDatabaseServerInfo databaseServerInfo)
        {
        ...
        {
            _logger.LogDebug(String.Format("DatabaseInventory.Execute called. MetricInstanceId: {0} Label: {1} Data: {2}", metricInstance, label, data));
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    _output = _logger.BuildErrorOutput("SqlServer2008.DatabaseInventoryPlugin", "Execute", _metricInstance, ex.ToString());
                    _dataQueuer.Queue(_output);
                }
                catch { }
        ...
        {
            var xml = new XDocument(new XElement("DatabaseInventoryPluginOutput",
            ...
            if (dataReader.FieldCount > 0)
            {
                while (dataReader.Read())
                {
                    ...
                    var element = new XElement("Database", new XAttribute("name", name));
